...
import StudentSkillAverages, {
  StudentSkillAverageView,
} from "@/components/results/StudentSkillAverages";
import { getCurrentUser } from "@/lib/auth";
import { getAccessibleStudentProfileIdsForParent } from "@/lib/guardianLinks";
import prisma from "@/lib/prisma";
import { calculateSkillAwardAverage } from "@/lib/resultAverages";
...
const ResultListPage = async ({ searchParams }: { searchParams: { [key: string]: string | undefined } }) => {
  ...
  const studentAverageSources = new Map<string, {
    student: Omit<StudentSkillAverageView, "totalScore" | "resultCount" | "average">;
  ...
  const skillAverages = Array.from(studentAverageSources.values())
    .map(({ student, results }) => ({
      ...student,
      ...calculateSkillAwardAverage(results),
    }))
  ...
  return <main className="min-h-full flex-1 bg-[#F4F7FB] p-3 text-[#0F172A] sm:p-5 lg:p-6"><section className="overflow-visible rounded-2xl border border-[#E2E8F0] bg-white shadow-[0_8px_24px_rgba(15,23,42,0.06)]">
    ...
    <StudentSkillAverages averages={skillAverages} personal={role === "student"} />
