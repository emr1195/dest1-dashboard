...
const TaskSubmissionsList = ({ items }: { items: TaskSubmissionItem[] }) => {
  ...
  return (
    <section aria-labelledby="submissions-title" className="min-w-0 max-w-full">
      <div className="flex min-w-0 max-w-full flex-col gap-3 border-b border-[var(--border-soft)] pb-4">
        <div className="flex min-w-0 flex-wrap items-center justify-between gap-2"><h3 id="submissions-title" className="min-w-0 flex-1 basis-full break-words text-lg font-extrabold text-[var(--text-primary)] sm:basis-auto">Respuestas de los muchachos</h3><span className="shrink-0 rounded-full bg-[var(--primary-soft)] px-3 py-1 text-xs font-bold text-[var(--primary)]">{items.length} {items.length === 1 ? "entrega" : "entregas"}</span></div>
        {items.length > 0 && <div className="grid min-w-0 gap-2 sm:grid-cols-3"><label className="sr-only" htmlFor="submission-search">Buscar estudiante</label><input id="submission-search" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar estudiante..." className="h-10 min-w-0 w-full rounded-xl border border-[var(--border-default)] px-3 text-sm outline-none focus:border-[var(--primary)] focus:ring-4 focus:ring-[var(--focus-ring)]" /><select aria-label="Filtrar entregas" value={status} onChange={(e) => setStatus(e.target.value)} className="h-10 min-w-0 w-full rounded-xl border border-[var(--border-default)] px-3 text-sm outline-none"><option value="">Todos los estados</option><option value="on-time">A tiempo</option><option value="late">Atrasadas</option><option value="pending">Pendientes de revisión</option><option value="reviewed">Evaluadas</option></select><select aria-label="Ordenar entregas" value={order} onChange={(e) => setOrder(e.target.value)} className="h-10 min-w-0 w-full rounded-xl border border-[var(--border-default)] px-3 text-sm outline-none"><option value="recent">Más recientes</option><option value="oldest">Más antiguas</option><option value="name">Nombre</option></select></div>}
      </div>
      {visible.length ? <div className="mt-3 min-w-0 space-y-2">{visible.map((item) => <article key={item.id} className="flex min-w-0 flex-col gap-3 overflow-hidden rounded-xl border border-[var(--border-soft)] bg-white p-3 transition hover:border-[var(--border-default)] hover:bg-[var(--surface-secondary)] sm:flex-row sm:items-center">
        ...
        <Link href={item.href} className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl border border-[var(--primary)] px-3 text-sm font-bold text-[var(--primary)] hover:bg-[var(--primary-soft)] focus:outline-none focus:ring-4 focus:ring-[var(--focus-ring)] sm:w-auto"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="h-4 w-4" aria-hidden="true"><path d="M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7S2 12 2 12Z" /><circle cx="12" cy="12" r="3" /></svg>Ver entrega</Link>
